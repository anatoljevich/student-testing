...
#check finished at
    ...
    ExamEntity.find_by_sql("SELECT (spec+number) AS name, correct, total, ROUND(correct*100/total) AS answer_percent, success_count, total_count, ROUND(success_count*100/total_count) AS total_percent FROM groups AS t5 JOIN ( SELECT sum(correct_questions) AS correct, sum(total_questions) AS total, group_id, sum(success_topics) AS success_count, sum(total_topics) AS total_count FROM ( SELECT sum(correct) AS correct_questions, sum(total) AS total_questions, t2.id, group_id, success_topics, total_topics, finished_at FROM exam_entity_results AS t1 JOIN exam_entities AS t2 ON t1.exam_entity_id = t2.id WHERE #{conditions} GROUP BY group_id, id ) AS t3 GROUP BY group_id ) AS t4 ON t5.id = t4.group_id ORDER BY total_percent DESC ; ")
